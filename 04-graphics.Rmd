...
Other important arguments to take note of are: `device =` where we tell `ggplot2` in what form we want the figure saved as (in this case saved as a .pdf); `units =` which specifies the units used in `width =` and `height =`; `wight =` and `height =` which specify the width and height of figure (in this case in mm as per `units =`); `dpi =` which controls the resolution of the saved figure; and `limitsize =` which prevents accidentally saving a figure 1 km x 1 km.
...
When using spatial data, heatmaps can be a great tool to visual spatial patterns. `ggplot2` can easily handle such data using `geom_bin2d()`. Even when not using spatial data, the figure shows where our data is most (and least) clustered.
...
In this case it doesn't look too pretty given that we don't have a wide spread of data (as we would hope for with spatial data).

### Hex map

A final version, similar to the previous "heat maps" is `geom_hex()`. Note that `geom_hex()` requires an additional package called `hexbin`.
