...
It is important to note here that `ggplot2` is not required to make "fancy" figures. If you'd prefer to use base R then go ahead. Almost, if not completely, equivalent figures are possible in base R. The difference between`ggplot2` and base is to do with how you _get_ to the end product rather than any differences in the end product itself. This is, never-the-less, a common belief almost certainly due to the fact that making a moderately attractive figure is, in my opinion at least, easier to do with `ggplot2` as, at the start, many decisions are made for the user, without you necessarily even knowing that a decision was ever made!
...
theme_deon <- function(base_size = 15, base_family = "", base_line_size = base_size / 22, base_rect_size = base_size / 22) {
  theme(
...
Before we go further, let's take a second and talk about what this figure is showing. On the y axes of the four plots we have the surface area taken up by a flower shoot, and the x axes show the weight of a flower. Each column of multiples (the name for each individual plot) shows the two treatments used in the experiment (whether the tip of the flower was cut or not) and the two rows show which block of the greenhouse the plants were grown in (block 1 or block 2). 

The differently coloured and shaped points represent the three nitrogen concentration levels used in the experiment. These colours are used to distinguish which points correspond to a certain nitrogen concentration. 

Additional to this we have four trend lines (produced by linear regressions, see Chapter 5). The three coloured solid lines show the trends of shoot area as a function of weight. These coloured lines are inturn specific to each "facet" in the grid, changing acording to treatment and block. The dashed black lines represent the shoot area as explained by the weight of the flower while ignoring any nitrogen affect. 
...
For the purposes of this chapter, we won't worry about the biology here. Do not take this to be standard, you should absolutely care deeply about the science in your own data. It is the science that should be the driving force behind the questions you ask, in turn determining what figures you want to make.
...
```{r installing & loading ggplot, eval = FALSE, message=F}
...
```{r pure ggplot, out.width = "75%", fig.align = "center", message=F}
ggplot()
```
...
Let's update our code to include the data and mapping layers (indicated by the grey Data and mustard Mapping _layer bubbles_ which will precede relevant code chunks):

<center>
![](images/data.png){ width=7.5% } ![](images/mapping.png){ width=7.5% }
</center>

```{r ggplot with xy axis, out.width = "75%", fig.align = "center", message=F}
...
<center>
![](images/geoms.png){ width=7.5% }
</center>

```{r ggplot with geom_point, out.width = "75%", fig.align = "center", message=F}
...
<center>
![](images/data.png){ width=7.5% } 

Data - the information we want to plot.

![](images/mapping.png){ width=7.5% } 
...
```{r geom_line, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea), data = flower) +
  geom_point() +
...
<center>
![](images/geoms.png){ width=7.5% } ![](images/statistics.png){ width=7.5% }
</center>

```{r geom_smooth, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea), data = flower) +
  geom_point() +
...
<center>
![](images/statistics.png){ width=7.5% }
</center>

```{r geom_smooth lm, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea), data = flower) +
  geom_point() +
...
<center>
![](images/mapping.png){ width=7.5% } ![](images/scales.png){ width=7.5% }
</center>

```{r geom_smooth colour lm, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea), data = flower) +
  geom_point() +
...
```{r universal vrs geom, out.width = "75%", fig.align = "center", message=F}
ggplot() +
...
```{r universal colours, out.width = "75%", fig.align = "center", message=F}
...
<center>
![](images/facets.png){ width=7.5% }`
</center>

```{r facet_wrap, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
...
```{r facet_wrap formula, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
...
```{r facet_grid, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
...
```{r facet_grid formula, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
...
```{r loading patchwork, eval = F, echo = T, message=F}
install.packages("patchwork")
library(patchwork)
```

```{r loading patchwork_true, eval = T, echo = F, message=F}
library(patchwork)
```

An important note: For those who have used base R to produce their figure and are familiar with using `par(mfrow = c(2,2))` (which allows plotting of four figures in two rows and two columns) be aware that this does not work for `ggplot2` objects. Instead you will need to use either the `patchwork` package or alternative packages such as `gridArrange` or `cowplot` or covert the `ggplot2` objects to grobs.
...
```{r named ggplot example, eval = FALSE, message=F}
p <- ggplot(df, mapping = aes(x = x, y = y))
```

And later to add additional layers, they would do:

```{r named ggplot example adding layer, eval = FALSE, message=F}
p + geom_point()
```
...
```{r calling figure name, out.width = "75%", fig.align = "center", message=F}
final_figure
```

We'll do the same with the figure we've created together:

```{r naming figure, out.width = "75%", fig.align = "center", message=F}
...
Now when the code is run, the figure won't be shown. This will only happen when we run the name of the figure as code. Well use this at the same time as showing how `patchwork` works.
...
```{r side-by-side patchwork, out.width = "85%", fig.align = "center", message=F}
...
```{r labelled nested, out.width = "75%", fig.align = "center", message=F}
...
<center>
![](images/scales.png){ width=7.5% } ![](images/mapping.png){ width=7.5% }
</center>

```{r change shape, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
...
```{r axis labels, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
	geom_point(aes(shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(method = "lm", se = FALSE) +
	facet_grid(block ~ treat) +
...
```{r bquote, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
	geom_point(aes(shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(method = "lm", se = FALSE) +
	facet_grid(block ~ treat) +
...
```{r extra labels, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
	geom_point(aes(shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(method = "lm", se = FALSE) +
	facet_grid(block ~ treat) +
...
```{r compact labels, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
	geom_point(aes(shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(method = "lm", se = FALSE) +
	facet_grid(block ~ treat) +
...
<center>
![](images/theme.png){ width=7.5% }
</center>

```{r testing themes, out.width = "75%", fig.align = "center", message=F}
...
```{r rbook figure, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
	geom_point(aes(shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(method = "lm", se = FALSE) +
	facet_grid(block ~ treat) +
...
```{r final figure reminder, out.width = "75%", fig.align = "center", message=F}
final_figure + 
	labs(title = "Reminder of the final figure")
```

Let's begin the final push by including that dashed horizontal line at the mean shoot area, about 80, on our y axis. This represents the overall mean area of a shoot, regardless of weight of flower, nitrogen concentration, treatment, or block. To draw a horizontal line we use a geom called `geom_hline()`, and the most important thing we need to specify is the y intercept value (in this case being our mean area of a shoot). We can also change the type of line using the argument `linetype =` and also the colour (as we did before). Let's see how it works.

<center>
![](images/geoms.png){ width=7.5% } ![](images/mapping.png){ width=7.5% }
</center>

```{r geom_hline, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
	geom_point(aes(shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(method = "lm", se = FALSE) +
	facet_grid(block ~ treat) +
...
```{r 2nd geom_smooth, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
	geom_point(aes(shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(method = "lm", se = FALSE) +
...
Choosing colours can be fiddly. I've found using a colour wheel helps with this step. You can always use Google to find a decent interactive colour wheel. Any decent website should give you a string, something like: #5C1AAE which are the "code" representations of those colours (technically called rgb specifications, which both R and `ggplot2` understand). Alternatively, there are colour names which R and `ggplot2` will also understand (e.g. "firebrick4"). Having chosen our colours using whichever means, let's see how we can do it:

<center>
![](images/scales.png){ width=7.5% }
</center>

```{r scale manual, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
	geom_point(aes(shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(method = "lm", se = FALSE) +
	geom_smooth(method = "lm", se = FALSE, linetype = 2, alpha = 0.6, colour = "black") +
...
```{r corrected factor level, out.width = "75%", fig.align = "center", message=F}
rbook_figure <- ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
	geom_point(aes(shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(method = "lm", se = FALSE) +
	geom_smooth(method = "lm", se = FALSE, linetype = 2, alpha = 0.6, colour = "black") +
...
```{r ggsave, eval = FALSE, message=F}
ggsave(filename = "areashoot_weight_facet.pdf", plot = rbook_figure, device = "pdf", path = "C:\\Users\\Nobel\\Prize",
...
The statistics layer is often ignored in favour of working solely with the geometry layer, as we have done above. The two are largely interchangeable, though the relative rareness of online help and discussions on the statistics layer seems to have relegated it to a state of anonymity. There is real value in at least understanding what the statistics layer is doing, even if you do not ever need to make direct use of it. It is perhaps most clear what the statistical layer is doing in the early `geom_smooth()` figure we made. 
...
Nowhere in our dataset are there columns for the y-intercept and gradient needed to draw the straight line, yet we've managed to draw one. The statistics layer calculates these based on our data, without us necessarily knowing what we've done. It is also the engine behind converting your data into counts for producing a bar chart, or densities for violin plots, or summary statistics for boxplots and so on. 

It is entirely possible that you will be able to use `ggplot2` for the vast majority of you plotting needs without ever consulting the statistical layer in any more detail than we have here (simply by "calling" - unknowingly - to it via the geometry layer), but be aware that it exists.

If we wanted to recreate the above figure using the statistical layer we would do it like this:

<center>
![](images/statistics.png){ width=7.5% }
</center>

```{r stat_smooth lm, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea), data = flower) +
  geom_point() +
...
While in this example it does not make a difference which we use, in other cases we may want to use the calculated statistics in alternative ways. We won't get into it, but see `?after_stat` if you are interested.
...
<center>
![](images/coords.png){ width=7.5% }
</center>


```{r axis limits, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea), data = flower) +
...
Notice the warning messages we get, telling us that n rows contain either missing or non-finite values? That's because we've essentially chopped out a huge part of our data using this method (everything outside of the ranges that we specified). As a result of doing this our lines have now completely changed direction. Notice that for low nitrogen concentration, the line is being drawn using only two points? This may, or may not be a problem depending on the aim we have, but we can use an alternative method; `coord_cartesian()`.
...
```{r coord_cartesian, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea), data = flower) +
	geom_point(aes(colour = nitrogen, shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(colour = "black", method = "lm", se = F, linetype = 2, alpha = 0.6) +
	geom_smooth(aes(colour = nitrogen), method = "lm", se = F, size = 1.2) +
...
```{r layering layers, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea), data = flower) +
	geom_smooth(aes(colour = nitrogen), method = "lm", se = F, size = 1.2) +
...
```{r continuous colour, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea), data = flower) +
	...
	labs(shape = "Nitrogen\nConcentration", colour = "Number of\nFlowers", caption = "Regression assumptions are unvalidated") +
	scale_shape_manual(values = c(15,17,19,21),
										 labels = c("High", "Medium", "Low")) +
	theme_rbook()
```
...
```{r scale_colour_gradient, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea), data = flower) +
	geom_point(aes(colour = flowers, shape = nitrogen), size = 2) +
	xlab("Weight of flower (g)") +
	ylab(bquote("Area of shoot"~(cm^2))) +
...
```{r scale_colour_gradient2, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea), data = flower) +
	geom_point(aes(colour = flowers, shape = nitrogen), size = 2) +
	xlab("Weight of flower (g)") +
	ylab(bquote("Area of shoot"~(cm^2))) +
...
```{r scale_colour_gradientn, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea), data = flower) +
	geom_point(aes(colour = flowers, shape = nitrogen), size = 2) +
	xlab("Weight of flower (g)") +
	ylab(bquote("Area of shoot"~(cm^2))) +
...
```{r cont point size, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea), data = flower) +
...
To move the position of the legend requires tweaking the theme, just as we did before with `theme_rbook()`. But for legends we might not want this to be set value whenever we use the theme (i.e. coding this into `theme_rbook()`). Instead we can change it on the fly depending on the individual figure. To do so, we can use a `theme()` layer and the argument `legend.position =`, followed swiftly by a layer specifying that we still want to use `theme_rbook()`.

<center>
![](images/theme.png){ width=7.5% }
</center>

```{r moving legend, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
	geom_point(aes(shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(method = "lm", se = FALSE) +
	geom_smooth(method = "lm", se = FALSE, linetype = 2, alpha = 0.6, colour = "black") +
...
```{r hiding legend, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
	geom_point(aes(shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(method = "lm", se = FALSE) +
	geom_smooth(method = "lm", se = FALSE, linetype = 2, alpha = 0.6, colour = "black") +
...
```{r hiding part of legend, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
...
```{r geom_text, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
	geom_point(aes(shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(method = "lm", se = FALSE) +
	geom_smooth(method = "lm", se = FALSE, linetype = 2, alpha = 0.6, colour = "black") +
...
```{r tag A, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
	geom_point(aes(shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(method = "lm", se = FALSE) +
	geom_smooth(method = "lm", se = FALSE, linetype = 2, alpha = 0.6, colour = "black") +
	...
	labs(shape = "Nitrogen\nConcentration", colour = "Nitrogen\nConcentration", caption = "Regression assumptions are unvalidated", tag = "A") +
...
```{r axis ticks, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
	geom_point(aes(shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(method = "lm", se = FALSE) +
	geom_smooth(method = "lm", se = FALSE, linetype = 2, alpha = 0.6, colour = "black") +
...
```{r rotate axis, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea, colour = nitrogen), data = flower) +
	geom_point(aes(shape = nitrogen), size = 2, alpha = 0.6) +
	geom_smooth(method = "lm", se = FALSE) +
	geom_smooth(method = "lm", se = FALSE, linetype = 2, alpha = 0.6, colour = "black") +
...
```{r density plot, out.width = "75%", fig.align = "center", message=F}
ggplot(flower) +
...
```{r histogram, out.width = "75%", fig.align = "center", message=F}
ggplot(flower) +
...
```{r freqpoly, out.width = "75%", fig.align = "center", message=F}
ggplot(flower) +
...
```{r boxplot, out.width = "75%", fig.align = "center", message=F}
ggplot(flower) +
...
```{r violin plot, out.width = "75%", fig.align = "center", message=F}
ggplot(flower) +
...
```{r barchart, out.width = "75%", fig.align = "center", message=F}
ggplot(flower) +
...
```{r quantile lines, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = weight, y = shootarea), data = flower) +
...
```{r heat, out.width = "75%", fig.align = "center", message=F}
...
```{r hex, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = leafarea, y = shootarea), data = flower) +
	geom_hex() +
	labs(y = "Shoot Area", x = "Leaf Area") +
	coord_cartesian(xlim = c(5,25)) +
...
```{r contour, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = leafarea, y = shootarea), data = flower) +
	geom_density2d() +
	labs(y = "Shoot Area", x = "Leaf Area") +
	coord_cartesian(xlim = c(5,25)) +
...
We can then expand on this using `stat()`. In the background of `ggplot2` values are calculated which are then passed to geoms. `stat()` acts like a flag to `ggplot2` that you want to use one of these calculated values. For instance, we can use the calculated "level" and use this to fill in our figure. To do so, we'll swap out `geom_density_2d()` for `stat_density_2d()` which will allow us to colour in the contour map.

```{r stat contour, out.width = "75%", fig.align = "center", message=F}
ggplot(aes(x = leafarea, y = shootarea), data = flower) +
...
```{r dotpot, out.width = "75%", fig.align = "center", message=F}
ggplot(flower) + 
...
ggpairs(flower, ggplot2::aes(colour = nitrogen), cardinality_threshold = NULL, progress = FALSE) +
	theme_rbook()
```
