...
That's exactly what we wanted, though it's still not matching the "final figure". We need `geom_smooth()` to draw lines for each level of nitrogen concentration. Getting `ggplot2` to do that is pretty straightforward. We can use the `colour =` argument within `aes()` (which means we want it displayed in the figure) to use tell `ggplot2` to draw a different coloured line depending on nitrogen concentration.
...
Instead of doing this within the specific geom, we'll use `scale_colour_manual()` as well as `scale_shape_manual()`. Doing it this way will allow to do two things as once; change the shape and colour to our choosing, and assign labels for these (much like what we did with `xlab()` and `ylab()`). Doing so is not too complex but will require nesting a function within another function, using `c()` (see chapter XXX for a reminder on the concatenate function). Let's see how we can do it:
...
ggplot(aes(x = weight, y = shootarea), data = flower) +
	...
	# Included value and label for Overall line
	scale_colour_manual(values = c("violetred3", "limegreen", "purple4", "black"),
											labels = c("High", "Medium", "Low", "Overall")) +
...
The reason for doing so is to maintain a single legend (though it is not ideal). If you'd prefer a perfectly accurate legend, then it would need to be broken into two legends. My preference is to have a single one, but as I said right at the beginning, this is subjective. You are hopefully now able to make a figure as you envisage.