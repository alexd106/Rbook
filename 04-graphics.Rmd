...
We'll need to go back to our previous code and do something clever. We can assign figures a name and then use that to call the figure whenever we run the name. For instance, I have assigned the "final figure" the name.... `final_figure` (I'm not a clever person). You may see this method used a lot in other textbooks, especially when adding addition layers (e.g. `final_figure + geom_point()`). I prefer not to do that here, instead favouring that the entire code is always visible. Regardless, let's remind ourselves of the final figure.
...
```{r rbook theme}
theme_rbook <- function(base_size = 15, base_family = "", base_line_size = base_size/22, base_rect_size = base_size/22) {         
  theme( 
    axis.title = element_text(size = 15),                               
    axis.text.x = element_text(size = 10),                              
    axis.text.y = element_text(size = 10),                              
