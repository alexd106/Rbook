...
For many people, one of their favourite uses of R is in making figures. These can either take the form of a quick dirty plot to allow you to get a feel for what's going on in your dataset, or fancier/more complex figures to use in a publication or report. This process is often as close as many scientists get to having a professional creative side (at least that's true for me), and it's something which those folks take great pride in.

As stated in the introduction, one of the many reasons for the rise in the popularity of R is due to its ability to produce publication standard figures (as well as those quick and dirty figures - which are the type you and I and everyone else will produce most of). Not only can R users make figures well suited for publication, but they are also offered a wide-range of customisation, in turn allowing users to create their own particular styles and brands of figures (well beyond the cookie-cutter styles in more traditional point and click programs). Because of this inherent flexibility when producing figures, the data visualisation side of R and supporting packages has grown substantially over the years.
...
Briefly, _Grammar of Graphics_ tried to move away from the idea that to create a scatterplot, a user should click the `scatterplot` button or use the `scatterplot()` function. Instead, by splitting figures into their various components (e.g. the underlying statistics, the geometric arrangement, the theme), a user will be able to manipulate each of these components (i.e. layers) and produce a tailor made figure fit for their very specific needs. Contrast this approach with the one used by, for example, excel. Think of how easy it is to spot an excel scatterplot. It's easy to spot because, for the most part, tweaking any or all of the layers in excel is difficult - you are at the mercy of the Microsoft Gods. 

_If you're wondering if you should buy the book, I'd strongly **not** recommend it. The book is focused on translating an "philosophy/approach" into a "programme". It's neither intended, nor suited, for teaching you how to make a figure._

While Wilkinson would eventually go on to become vice-president of SPSS, his (and his oft forgotten co-author's) ideas would, never-the-less, make their way into R via `ggplot2`. 
...
final_figure <- ggplot(aes(x = weight, y = shootarea), data = df) +
  ...
  labs(shape = "Nitrogen\nConcentration", 
  		 colour = "Nitrogen\nConcentration", 
  		 caption = "Lines are only to suggest trends") +
  scale_colour_manual(
...
```{r loading patchwork, eval = T, echo = T}
...
```{r named ggplot example, eval = FALSE}
p <- ggplot(df, mapping = aes(x = x, y = y))
```

And later to add additional layers, they would do:

```{r named ggplot example adding layer, eval = FALSE}
p + geom_point()
```
...
ggpairs(flower, ggplot2::aes(colour = nitrogen), cardinality_threshold = NULL, progress = FALSE, bins = 30) +
	theme_rbook()
```
