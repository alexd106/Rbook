...
Next, let's investigate the relationship between `mortality` and `smoking` variables by plotting a scatterplot. We can use either the `ggplot2` package or base R graphics to do this. We'll use `ggplot2` this time. 
...
Another approach to interpreting your model output is to plot a graph of your data and then add the fitted model to this plot. Let's go back to the first linear model we fitted (`smoke_lm`). We can add the fitted line to our previous plot using the ``ggplot2` package.`geom_smooth` geom and also include the standard errors by specifying the `se = TRUE` argument.
