...
OK, so how does this work? Well, the logical test is `flowers$height > 12` and R will only extract those rows that satisfy this logical condition. If we look at the output of just the logical condition you can see this returns a vector containing `TRUE` values if `height` is greater than 12 and `FALSE` if `height` is not greater than 12.

```{r dw13, echo=TRUE, collapse=TRUE}
flowers$height > 12
```

So our row index is a vector contain either `TRUE` and `FALSE` values and only those rows that are `TRUE` are selected. Remember, when we don't select columns to extract (nothing after the comma) all columns are returned by default.

The other commonly used operators are shown below
...
Or `nitrogen` level is not equal to `medium` and only return columns 1 to 4. 
...
or perhaps descending value of `leafarea`
...
We can even order data frames based on multiple variables. For example, to order `flowers` in ascending order of both `block` and `height`
...
If we wanted to do the same thing with a character (or factor) variable like `nitrogen` we would need to use another function `xtfrm()` for this variable.
...
Notice that the `nitrogen` variable had been reverse ordered alphabetically and `height` has been ordered by increasing values within each level of `nitrogen`. 

### Adding columns and rows

Sometimes it's useful to be able to add extra rows and columns to our data frames. There are multiple ways to achieve this (as there always is in R!) depending on your circumstances. To simply append additional rows to an existing data frame we can use the `rbind()` function and to append columns the `cbind()` function. Let's create couple of test data frames to see this in action using our old friend the `data.frame()` function.
...
Because both of our data frames contains contains at least one variable in common (`species` in our case) we can simply use the `merge()` function to create a new data frame called `taxa_zone`
...
Notice that the merged data frame contains only the rows that have both `species` information in **both** data frames. There are also two columns called `GENUS` and `genus` because the `merge()` function treats these as two different variables that originate from the two data frames. 
...
And the same data in the wide format. In this format we have multiple observations from a subject in a single row with each observation in a different column. This is a common format when you have repeated measurements from sampling units.
...
There are many ways to convert between these two formats but we'll use the `melt()` and `dcast()` functions from the `reshape2` package (you will need to install this package first). The `melt()` function is used to convert from wide to long formats. The first argument for the `melt()` function is the data frame we want to melt (in our case `wide_data`). The `id.vars =` argument is vector of the variables you want to stack, the `measured.vars =` argument identifies the columns of the different conditions, the `variable.name =` argument specifies what you want to call the stacked column of your different conditions and `value.name =` is the name of the column of your stacked measurements.
