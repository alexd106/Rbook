...
In the code above we have indexed the columns we want to summarise in the `flowers` data frame using `flowers[, 4:7]`. The `by =` argument specifies a list of factors (`list(mean_nitrogen = flowers$nitrogen)`) and the `FUN =` argument names the function to apply (`mean` in this example).
...
Or only those plants in `block` 1.
