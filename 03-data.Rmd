...
We can even use negative positional indexes to exclude certain rows and columns. As an example, lets extract all of the rows except the first 85 rows and all columns except the 4^th^, 7^th^ and 8^th^ columns. Notice we need to use the `-c()` function even though we have generated a vector for the rows using the `:` method. If we just use `-1:85` this would actually generate a regular sequence from -1 to 85 and not what we want (we can of course use `-1:-85`).

```{r dw9, echo=TRUE, collapse=TRUE}
flowers[-c(1:85), -c(4, 7, 8)]
```
...
Just as we did with vectors, we can also extract data from our data frame based on a logical test. Let's extract all rows where `height` is greater than 12 and extract all columns by default.
...
To extract rows based on an `OR` Bollean expression we can use the `|` symbol. Let's extract all rows where `height` is greater than 12.3 OR less than 2.2
