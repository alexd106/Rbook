...
Just as we did with [vectors](#vectors), we also can access specific data in data frames using the square bracket `[ ]` notation. However, instead of just using a single index, we now need to use two indexes, one to specify the rows and one for the columns. To do this, we can use the notation `my_data[rows, columns]` where `rows` and `columns` are indexes and `my_data` is the name of the data frame. Again, just like with our vectors our indexes can be positional or the result of a logical test. 
...
We can also extract values from multiple rows or columns by specifying these indexes as vectors inside the `[ ]`. So, to extract the first 10 rows and the first 4 columns we simple supply a vector containing a sequence from 1 to 10 for the rows index (`1:10`) and a vector from 1 to 4 for the column index (`1:4`).

```{r dw5, echo=TRUE, collapse=TRUE}
flowers[1:10, 1:4]
```

Or if our row and columns are not sequential then we can supply vectors of positions using the `c()` function. To extract the 1^st^, 5^th^, 12^th^, 30^th^ rows from the 1^st^, 3^rd^, 6^th^ and 8^th^ columns
...
Notice in the code above that we need to use the `flowers$height` notation for the logical test. If we only named the `height` variable without the name of the data frame we would receive an error telling us R couldn't find the variable `height`. The reason for this is that the `height` variable only exists inside the `flowers` object so you need to tell R exactly where it is.
...
We can also order by descending order of a variable (i.e. `leafarea`) using the `decreasing =` argument. 
...
If we wanted to do the same thing with a character (or factor) variable like `nitrogen` we would need to use the function `xtfrm()` for this variable inside our `order()` function.
...
Sometimes it's useful to be able to add extra rows and columns to our data frames. There are multiple ways to achieve this (as there always is in R!) depending on your circumstances. To simply append additional rows to an existing data frame we can use the `rbind()` function and to append columns the `cbind()` function. Let's create a couple of test data frames to see this in action using our old friend the `data.frame()` function.
...
Instead of just appending either rows or columns to a data frame we can also merge two different data frames together. let's say we have one data frame that contains taxonomic information on some common rocky shore invertebrates and another data frame that contains information on where they are commonly found on the rocky shore. We can merge these two data frames together to produce a single data frame with both taxonomic and location information. Let's first create both of these data frames (in reality you would probably just import your different datasets).
...
If the variable names that you want to base the merge on are different in each data frame (in our example `GENUS` and `genus`) you can specify the names in the first data frame (known as `x`) and the second data frame (known as `y`) using the `by.x =` and `by.y =` arguments.
...
Reshaping data into different formats is a common task. With rectangular type data (same number of rows in each column) there are two main data frame shapes that you will come across: the 'long' format (sometimes called stacked) and the 'wide' format. An example of a long format data frame is given below. We can see that each row is a single observation from an individual subject and each subject can have multiple rows.  
...
We can also format the same data in the wide format as shown below. In this format we have multiple observations from each subject in a single row with observations in different columns. This is a common format when you have repeated measurements from sampling units.
...
Whilst there's no inherent problem with either of these formats we will often need to convert between the two as some functions will require a specific format for them to work. The most common format (and arguably the most useful) is the long format.

There are many ways to convert between these two formats but we'll use the `melt()` and `dcast()` functions from the `reshape2` package (you will need to install this package first). The `melt()` function is used to convert from wide to long formats. The first argument for the `melt()` function is the data frame we want to melt (in our case `wide_data`). The `id.vars =` argument is a vector of the variables you want to stack, the `measured.vars =` argument identifies the columns of the measurements in different conditions, the `variable.name =` argument specifies what you want to call the stacked column of your different conditions in your output data frame and `value.name =` is the name of the column of your stacked measurements again in your output data frame.
...
The `dcast()` function is used to convert from a long format data frame to a wide format data frame. The first argument is again is the data frame we want to cast (`long_data` for this example). The second argument is in formula syntax. The `subject + age` bit of the formula means that we want to keep these columns separate, and the `~ condition` is the column that contains the labels that we want to split into new columns in our new data frame. The `value.var =` argument is the column that contains the actual measurements  

```{r dw37, echo=TRUE, collapse=TRUE}
long_data   # remind ourselves what the long format look like

# convert long to wide
my_wide_df <- dcast(data = long_data, subject + sex ~ condition, value.var = "measurement")
my_wide_df
```
...
Continuous variables (i.e. `height`, `weight` etc) are summarised as the mean, minimum, maximum, median, first (lower) quartile and third (upper) quartile. For factor variables (i.e. `treat` and `nitrogen`) the number of observations in each of the factor levels is given. If a variable contains missing data then the number of `NA` values is also reported.
...
We can also summarise our data for each level of a factor variable. Let's say we want to calculate the mean value of `height` for each of our `low`, `meadium` and `high` levels of `nitrogen`. To do this we will use the `mean()` function and apply this on the `height` variable for each level of `nitrogen` using the `tapply()` function.

```{r sum9, echo=TRUE, collapse=TRUE}
tapply(flowers$height, flowers$nitrogen, mean)
```

The `tapply()` is not just restricted to calculating mean values, you can use it to apply other functions or even functions you've written yourself. For example, we can apply the `sd()` function to calculate the standard deviation for each level of `nitrogen`.

```{r sum10, echo=TRUE, collapse=TRUE}
tapply(flowers$height, flowers$nitrogen, sd)
```

Note, if the variable you want to summarise contains missing values (`NA`) you will also need to include an argument specifying how you want the function to deal with the `NA` values. We saw an example if this in [Chapter 2](#na_vals) where the `mean()` function returned an `NA` when we had missing data. 
...
Another really useful function for summarising data is the `aggregate()` function. The `aggregate()` function works in a very similar fashion to `tapply()` (actually, `aggregate()` is a convenience function for `tapply()`) but is a bit more flexible. 
...
We can also use the `aggregate()` in a different way by using the formula method (as we did with `xtabs()`). The formula method specifies the the variable to apply the function on the left hand side of the `~` and the factors on the right hand side separated by a `+` symbol. The formula method also allows you to use the `data =` argument for convenience.  

```{r sum16, echo=TRUE, collapse=TRUE}
aggregate(height ~ nitrogen + treat, FUN = mean, data = flowers)
```

One advantage of using the formula method is that you can also use the `subset =` argument to apply the function to subsets of the original data. For example, to calculate the mean `height` for each combination of the `nitrogen` and `treat` factors but only for those plants that have less than 7 `flowers`.
...
By now hopefully you're getting a feel for how powerful and useful R is for manipulating and summarising data (and we've only really scratched the surface). One of the great benefits of doing all your data wrangling in R is that you now have a permanent record of all the things you've done to your data. No more making undocumented changes in Excel or Calc! By treating your data as 'read only' and documenting all of your decisions you will have made great strides towards making your analysis more reproducible and transparent to others. It's important to realise, however, that any changes you've made to your data frame in R will not change the original data file you imported into R (and that's a good thing). Happily it's straight forward to export data frames to external files in a wide variety of formats.

The main workhorse function for exporting data frames is the `write.table()` function. As with the `read.table()` function, the `write.table()` function is very flexible with lots of arguments to help customise it's behaviour. Let's take our original `flowers` data frame, order the rows in ascending order of `height` within each level `nitrogen`. We also want to apply a square root transformation on the number of flowers variable (`flowers`) and a log~10~ transformation on the `height` variable and save these as additional columns in our data frame. 
...
As we saved the file as a tab delimited text file we could open this up in any text editor. Perhaps a more familiar option would be to open the file in Excel by selecting `File` -> `Open ..` in the main menu and then select the file. Choose the 'Tab' option to set the delimiter and then click on the 'Finish' button.
...
We can of course export our files in a variety of formats. Another popular option is to export files in csv (comma separated values) format. We can do this using the `write.table()` function by setting the separator argument to `sep = ","`.
