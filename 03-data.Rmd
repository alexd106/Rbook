...
For example if for some reason we wanted to extract the first value (1^st^ row ) of our `height` variable (4^th^ column)

```{r dw4, echo=TRUE, collapse=TRUE}
flowers[1, 4]
```

We can also extract values from multiple rows or columns by specifying these as indexes. To extract the first 10 rows and the first 4 columns
...
```{r dw8, echo=TRUE, collapse=TRUE}
flowers[, 1:3]
```

We can even use negative positional indexes to exclude certain row and columns. As an example, lets extract all of the rows except the first 70 rows and all columns except the 4^th, 7^th^ and 8^th^ columns. Notice we need to use the `-c()` function even though we have generated a vector for the rows using the `:` method. If we just use `-1:70` this would actually generate a regular sequence from -1 to 70 - not what we want!

```{r dw9, echo=TRUE, collapse=TRUE}
flowers[-c(1:70), -c(4, 7, 8)]
```

In addition to using a position index for extracting particular variables (columns) you can also name the variables directly when using the square bracket `[ ]` notation. For example, let's extract the first 5 rows and the variables `treat`, `nitrogen` and `leafarea`. Instead of using `flowers[1:5, c(1, 2, 6)]` we can use

```{r dw10, echo=TRUE, collapse=TRUE}
flowers[1:5, c("treat", "nitrogen", "leafarea")]
```

We often use this method in preference to the positional index as it will still give us what we want even if the position of the variables change. 
