...
It's also often useful to define row and column names for your matrix (but not required)
...
Once you've created your matrices we can do useful stuff with them. As you'd expect, R has numerous built in functions to perform matrix operations. Some of the most common are given below. For example, to transpose a matrix we use the transposition function `t()`
...
To extract the diagonal elements of the matrix and store them as a vector we can use the `diag()` function
...
And matrix addition, multiplication etc can be performed 
