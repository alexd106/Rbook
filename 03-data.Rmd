...
We can construct a data frame from existing data objects such as vectors using the `data.frame()` function. As an example, lets create three vectors `height`, `weight` and `p.names` and include all of these vectors in a data frame object called `dataf`.
...
But much more common way to create a data frame is to import data from an external file, something we will cover in the next section. 
