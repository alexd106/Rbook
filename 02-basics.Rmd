...
Manipulating, summarising and sorting data using R is an important skill to master but one which many people find a little confusing at first. We will go through a few simple examples here to illustrate some important concepts but will build on this in much more detail in [Chapter 3](#data_r). 

To extract some of the values (more generally known as elements) from a vector we should use the square bracket `[ ]` notation. The general approach is to name the object you wish to extract from, then a set of square brackets with the position of the element contained within the square brackets. For example, to extract the 3rd value of the vector `my_vec` we created before 
...
or extract a range of values using the `:` notation
