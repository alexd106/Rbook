...
We now want to calculate the mean temperature over these days using the `mean()` funtion
...
OK, what the heck! Why does the `mean()` function return an NA? Actually, R is doing something very sensible (at least in our opinion!). If a vector has a missing value (`NA`) then the only possible value to return when calculating a mean is `NA`. R doesn't know that you perhaps want to ignore the `NA` values (R can't read your mind - yet!). Happily, if we look at the help file (see the [next section](#help) for more details) associated with the `mean()` function we can see there is an argument `na.rm = FALSE`. If we change this to `na.rm = TRUE` this will allow us to ignore the `NA` values when calculating the mean 
...
The point of the above is to highlight how we can change the default behaviour of a function using an appropriate argument. The problem is that not all functions will have an `na.rm =` argument, they will deal with `NA` values differently. However, the good news is that every help file associated with any function will **always** tell you how missing data are handled by default.
...
Admittedly the help files can seem anything but helpful when you first start using R. This is probably because they're written in a very concise manner and the language used is often quite technical and full of jargon. Having said that, you do get used to this and will over time even come to appreciate a certain beauty in their brevity (honest!). One big advantage of an R help file is that the structure is consistent no matter what function you get help with. This makes it easy to navigate through the file to find exactly what you need.  Each help file will contain the following sections:   
