...
Let's start with the axis labels. To add labels to the y axis and x axis we use the corresponding `ylab =` and `xlab =` arguments. Both of these arguments need character stings as values.
...
But now we have a new problem, the top of the y axis label gets cut off. To remedy this we need to adjust the plot margins using the `par()` function and the `mar =` argument before we plot the graph. The `par()` function is the main function for setting graphical parameters in base R and the `mar =` argument sets the size of the margins that surround the plot. You can adjust the size of the margins using the notation `par(mar = c(bottom, left, top, right)` where the arguments `bottom`, `left`, `top`, `right` are the size of the margins. By default R sets these margins as `mar = c(5.1, 4.1, 4.1, 2.1)` with these numbers corresponding to the number of lines in each margin. Let's increase the size if the left margin a little bit and decrease the size of the right margin by a smidge.    
