...
The most common high level function used to produce plots in R is (rather unsurprisingly) the `plot()` function. For example, to plot the `weight` of petunia plants in our `flowers` data frame which we imported in [Chapter 3](#import_fnc).
...
If we'd only specified the variable `weight` rather than `flowers$weight`, the `plot()` function will throw an error as the variable `weight` only exists in the `flowers` data frame object.
...
To plot a scatterplot of one numeric variable against another numeric variable we just need to include both variables as arguments when using the `plot()` function. For example to plot `shootarea` on the y axis and `weight` of the x axis from our `flowers` data frame. 
...
Or perhaps we would like a boxplot of our `weight` variable for each level of `nitrogen` using the `bwplot()` function.
...
We can also include more than one conditioning variables. For example, let's create a scatter plot of `height` against `weight` for each level of `nitrogen` and `treat`. To do this we'll use the `xyplot()` lattice function.
...
But now we have a new problem, the very top of the y axis label gets cut off. To remedy this we need to adjust the plot margins using the `par()` function and the `mar =` argument before we plot the graph. The `par()` function is the main function for setting graphical parameters in base R and the `mar =` argument sets the size of the margins that surround the plot. You can adjust the size of the margins using the notation `par(mar = c(bottom, left, top, right)` where the arguments `bottom`, `left`, `top` and `right` are the size of the margins. By default R sets these margins as `mar = c(5.1, 4.1, 4.1, 2.1)` with these numbers corresponding to the number of lines in each margin. Let's increase the size if the left margin a little bit and decrease the size of the right margin by a smidge.    
...
We can also change the type of plotting symbol, the colour of the symbol and the size of the symbol using the `pch =` and `col =` and `cex =` arguments respectively. The `pch =` argument takes an integer value, between 0 and 25 to specify the type of plotting symbol. Symbols 0 to 14 are open symbols, 15 to 20 are filled symbols and 21 to 25 are symbols where you can specify a different fill colour and outside line colour. Here's a summary table displaying the value and corresponding symbol type. 
...
The `col =` argument changes the colour of the plotting symbols. This argument can either take an integer value to specify the colour or a character string giving the colour name. To see a list of all the 657 preset colours available in base R use the `colours()` function (you can also use `colors()`) or perhaps even easier see this [link][colours]. More colour options are available with other packages (see the excellent `RColorBrewer` package) or you can even 'mix' your own colours using the `colorRamp()` function (see `?colorRamp` for more details).  

The `cex =` argument allow you to change the size of the plotting symbol. This argument works in the same way as some of the other `cex` arguments we have already seen (i.e. `cex.axis`) and requires a numeric value to indicate the increase or decrease in size relative to the default value of 1. 
...
For even more control over how your plot looks we can build our plot up in layers, customising each step as we go along. For example, perhaps we want to create a plot of `shootarea` and `weight` as we did before but this time we want to change the symbol colours of our data points depending on what level of `nitrogen` the plants were exposed to. The general approach is that we use our high level plotting function `plot()` to create the general plot (axes, axes labels etc) but we don't actually plot the data at this point. We then use a low level function `points()` to add the plotting symbols one `nitrogen` level at a time whilst customising the look of the symbols. Let's go through this approach a set at a time. First we'll make the plot but suppress plotting the data using the `type = "n"` argument in the `plot()` function. 
...
We can now use the `points()` function again to plot data from the  `medium` level of nitrogen and change the symbol colour to something different. Notice that we do not reuse the `plot()` function here as we are just using the low level function `points()` to add data points to the existing plot.
...
And add the `high` level of `nitrogen` data to the plot and add our text label to the plot as before.
...
The only thing left to do is add a legend to the plot to let your reader know what what `nitrogen` level each of the colours corresponds to. We'll use another low level function,`legend()` to do this. The `legend()` function requires us to provide the x and y coordinates to specify the position of the top left of the legend in the plot, a vector of colours, symbol types and labels to use in the legend. The `bty = "n"` argument stops a border being drawn around the legend and the `title =` argument gives the legend a title.
...
```{r plot54, echo=TRUE, out.width="75%", fig.align="center"}
par(mar = c(4.1, 4.4, 4.1, 1.9), xaxs="i", yaxs="i")
plot(flowers$weight, flowers$shootarea,
       type = "n",
       xlab = "weight (g)",
...
There are a number of different methods for plotting multiple graphs within the same graphics device, some of which you've already met such `pairs()`, `coplot()`, `xyplot()` etc. However these functions rely on plotting multiple graphs in different panels within the same plot. If you want to plot separate plots within the same graphics device you will need a different approach. One of the most common methods is to use the main graphical function `par()` to split the plotting device up into a number of defined sections using the `mfrow =` argument. With this method, you first need to specify the number of rows and columns of plots you would like and then run the code for each plot. For example, to plot two graphs side by side we would use `par(mfrow = c(1, 2))` to split the device into 1 row and two columns.
...
A more flexible approach is to use the `layout()` function. The `layout()` function allows you to split your plotting device up into different sized regions and can be used to build complex figures. Before using the `layout()` function we first need to specify how we are going to split up out plotting device by creating a matrix using the `matrix()` function. Let's create a 2 x 2 matrix.
...
We can now use the `layout()` function to define our layout. As we have two rows and two columns we need to specify the height of each row and the width of each column using the `heights =` and `widths =` arguments. The `respect = TRUE` argument ensures that the units used to define the widths are the same as those to define the heights. We can get a graphical representation of our layout by using the `layout.show()` function. 
...
All we need to do now is create our three plots. However, before we do this we also need to change the figure margins for each of the figures using the `par(mar = )` command so all of the plots can fit together in the same plotting device. this will probably take a little bit of experimenting to get the plot looking exactly how you want. For our first figure (bottom left) we will reduce the bottom and left margin a little bit and remove the margins completely from the top and right sides with `par(mar = c(4, 4, 0, 0))`. For our top plot we will remove the margins from the bottom, top and right sides and set the left side to have the same margin as our first figure (`par(mar = c(0, 4, 0, 0))`). For the third plot on the right we will set the bottom side to have the same margin as the first plot so they line up and remove the margins from the other sides with `par(mar = c(4, 0, 0, 0))`.  

```{r plot59, echo=TRUE, out.width="75%", fig.align="center"}
par(mar = c(4, 4, 0, 0))
plot(flowers$weight, flowers$shootarea, 
...
By default, R (and therefore RStudio) will direct any plot you create to the plot window. To save your plot to an external file you first need to redirect your plot to a different graphics device. You do this by using one of the many graphics device functions to start a new graphic device. For example, to save a plot in pdf format we will use the `pdf()` function. The first argument in the `pdf()` function is the filepath and filename (with .pdf extension) of the file we want to save. Once we've used the `pdf()` function we can then supply all of the code we used to create our plot including any graphical parameters such as setting the margins and splitting up the plotting device. Once the code has run we need to close the pdf plotting device using the `dev.off()` function. 
...
If we wanted to save this plot in png format we simply use the `png()` function in more or less the same way we used the `pdf()` function.
