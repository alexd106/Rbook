...
To install a package from CRAN you can use the `install.packages()` function. For example if you want to install the `devtools` package enter the following code into the console window of RStudio (note: you will need a working internet connection to do this)
...
This is just one example and there are no hard and fast rules so feel free to develop a system that works for you. A really useful shortcut in RStudio is to automatically include a time and date stamp in your R script. To do this, write `ts` where you want to insert your time stamp and then press the 'shift + tab' keys. RStudio will magically convert `ts` into the current date and time and also automatically comment out this line with a `#`. Another really useful RStudio shortcut is to comment out multiple lines in your script with a `#` symbol. To do this, highlight the lines of text you want to comment and then press 'ctrl + shift + c'. To uncomment the lines just use 'ctrl + shift + c' again.  
...
```{r styler, echo=FALSE, out.width="75%", fig.align="center"}
