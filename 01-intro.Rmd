...
There are multiple options for installing packages hosted on GitHub. Perhaps the most common method is to use the `install_github()` function from the `devtools` package (you installed this package [previously](#cran_packages). Before you use the function you will need to know the GitHub user name of the repository owner and also the name of the repository. For example, to install the development version of `dplyr` from Hadley Wickham the repository owner username is 'tidyverse' and the repository name is 'dplyr' (just Google 'github dplyr')
